...
class PyDriveSharedBackend(duplicity.backend.Backend):
    u"""Connect to remote shared store using PyDrive API"""

    def __init__(self, parsed_url):
        duplicity.backend.Backend.__init__(self, parsed_url)

        if parsed_url.query:
            self.shared_drive_id = parsed_url.query
        else:
            raise BackendException(u"""\
PyDriveShared backend requires the shared drive ID to be passed in as if it were
a URI query (after the ? character). The ID is the string of characters at the 
end of the URL when you select the shared drive in the Google web interface.
Example usage:
    duplicity full /source-folder/ pydriveshared://developer.gserviceaccount.com/target-folder/?<SHARED DRIVE ID>
""")
...
Exception: %s""" % str(e))
        ...
        if u'GOOGLE_DRIVE_ACCOUNT_KEY' in os.environ:
            ...
            # https://github.com/googleapis/google-api-python-client/pull/813/files#diff-84a6c030bfdfe3ce2eac14a3369e6114
            http = httplib2.Http()
        ...
        # Root folder id is the shared drive id
        ...
        for folder_name in folder_names:
            ...
            file_list = self.drive.ListFile({u'q': u"'" + parent_folder_id +
                                             ...
                                             u'supportsTeamDrives': True}).GetList()
    ...
    def file_by_name(self, filename):
        ...
        flist = self.drive.ListFile({u'q': q,
                                    u'fields': fields,
                                    u'corpora': 'teamDrive',
                                    u'teamDriveId': self.shared_drive_id,
                                    u'includeTeamDriveItems': True,
...
duplicity.backend.register_backend(u'pydriveshared', PyDriveSharedBackend)
u""" tweaked version of pydrive backend that works on shared drives """

duplicity.backend.uses_netloc.extend([u'pydriveshared'])
