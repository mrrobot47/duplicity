...
def get_delta_path(new_path, sig_path, sigTarFile=None):
    ...
    def callback(sig_string):
        ...
        ti.name = u"signature/" + u"/".join(index)
    ...
    if new_path.isreg() and sig_path and sig_path.isreg() and sig_path.difftype == u"signature":
        ...
        if sigTarFile:
            ti.name = u"snapshot/" + u"/".join(index)
...
def sigtar2path_iter(sigtarobj):
    ...
    for tarinfo in tf:
        ...
        for prefix in [u"signature/", u"snapshot/", u"deleted/"]:
...
class TarBlockIter:
    ...
    def tarinfo2tarblock(self, index, tarinfo, file_data=u""):
        u"""
        ...
        if remainder > 0:
            filler_data = u"\0" * (tarfile.BLOCKSIZE - remainder)
        else:
            filler_data = u""
        return TarBlock(index, u"%s%s%s" % (headers, file_data, filler_data))
...
class DeltaTarBlockIter(TarBlockIter):
    ...
    def process(self, delta_ropath):
        ...
        def add_prefix(tarinfo, prefix):
            ...
            if tarinfo.name == u".":
                tarinfo.name = prefix + u"/"
            else:
                tarinfo.name = u"%s/%s" % (prefix, tarinfo.name)
